...
incProgressUnfixableErrors :: (Num a, Show a) => Progress a -> Progress a
incProgressUnfixableErrors Progress{..} = Progress{ pErrorsUnfixable = pErrorsUnfixable + 1
                                                  , ..
                                                  }

-- | Increase the number of fixable errors.
incProgressFixableErrors :: (Num a, Show a) => Progress a -> Progress a
...
decProgressFixableErrors :: (Num a, Show a) => Progress a -> Progress a
decProgressFixableErrors Progress{..} = Progress{ pErrorsFixable = pErrorsFixable - 1
                                                , ..
                                                }

fixableToUnfixable :: (Num a, Show a) => Progress a -> Progress a
...
showProgress name width col posixTime Progress{..} = mconcat
  ...
  where
    ...
    errsU = ceiling $ (pErrorsUnfixable % pTotal) * fromIntegral width
    ...
    errsF = min (width - errsU) . ceiling $ (pErrorsFixable % pTotal) * fromIntegral width
