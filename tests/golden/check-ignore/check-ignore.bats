...
@test "Ignore file with broken xrefcheck annotation: directory, check failure" {
  ...
  to_temp xrefcheck -u --ignore ./to-ignore/inner-directory/
  assert_diff
}
...
@test "Ignore referenced file, check error" {
  ...
  to_temp xrefcheck -u --ignore referenced-file.md

  assert_diff
}

@test "Config: Absolute fiepath in \"ignore\" error" {
  run xrefcheck -u --config ./config-ignore-bad-path-absolute.yaml
...
@test "Config: Malformed glob in \"ignore\" error" {
  run xrefcheck -u --config ./config-ignore-malformed-glob.yaml
