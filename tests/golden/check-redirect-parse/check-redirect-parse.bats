...
@test "No redirect rules" {
  ...
  to_temp xrefcheck -u -c no-rules.yaml
  assert_diff
}

@test "Only outcome" {
  golden_file=$(realpath expected2.gold)
  to_temp xrefcheck -u -c only-outcome.yaml
  assert_diff
}

@test "Only outcome and to" {
  golden_file=$(realpath expected3.gold)
  to_temp xrefcheck -u -c only-outcome-to.yaml
  assert_diff
}
